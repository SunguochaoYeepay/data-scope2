...
export const useQueryStore = defineStore('query', () => {
  ...
  const fetchQueries = async (params?: FetchQueryParams) => {
    ...
    try {
      // 使用queryService的getQueryHistory方法
      const historyParams = {
        dataSourceId: params?.dataSourceId,
        queryType: params?.queryType,
      ...
      const result = await queryService.getQueryHistory(historyParams);
      queries.value = result.items;
      return queries.value;
    } catch (err) {
      error.value = err instanceof Error ? err : new Error(String(err));
      message.error(`获取查询列表失败: ${error.value.message}`);
      throw error.value;
    }
  };
  ...
  const fetchQueryById = async (id: string) => {
    ...
    try {
      // 使用queryService的getQuery方法
      const query = await queryService.getQuery(id);
      
      if (!query) {
        throw new Error(`查询ID ${id} 不存在`);
      }
      ...
      error.value = err instanceof Error ? err : new Error(String(err));
      message.error(`获取查询详情失败: ${error.value.message}`);
      throw error.value;
    }
  };
